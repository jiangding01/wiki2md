...
- **v2.0（本次）**：适配器架构、通用兜底、Confluence 评论、5 个精配站点、图片 ZIP、
  预览页、右键节选、快捷键、设置页。
- **v2.1**：飞书接口化采集（block 树 → IR，替代滚动采集）；飞书评论；导出历史记录。
- **v2.2**：腾讯文档专项（内部导出接口）；Notion / 语雀 / Google Docs 适配器。
- **v2.3**：批量导出（Confluence 空间树 / 飞书知识库整库）；自定义 Turndown 规则；
  MathJax/KaTeX 通用公式还原。
- **v3.0**：适配器插件化——用户可用「选择器 + 规则 JSON」自定义站点适配，社区共享。
